...
`)
          ...
          if (collectedMessage) {
            ...
            if (feedbackChannel) {
              ...
              interaction.followUp({ content: `**sarkawtw Bw :white_check_mark:  **` });
           
            }
            let roleID = await db.get(`role_${interaction.guild.id}_${client.user.id}`, '1197495044408610817') //لێرە ئایدی ڕۆڵەکە دابنێ
            let role = interaction.guild.roles.cache.find(x => x.id === roleID)
            db.set(`role_${interaction.guild.id}`, roleID)
            if (role) {
